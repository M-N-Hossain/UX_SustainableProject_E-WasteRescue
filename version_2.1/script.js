...
function initializeScripts() {
  ...
  const actionBtn = document.getElementById("take-action-btn");
  ...
  actionBtn.addEventListener("click", function (e) {
    ...
    if (clickCount < 2) {
      alert(`Hi! I'm your unexpected friendly advertise`);
    } else if (clickCount === 2) {
      alert(`Ah ha! Here me again! your unexpected friendly advertise`);
    } else {
      clickCount = 0;
      alert(
        `Well done! You're a true e-waste warrior! We respect your commitment. Lets fight together!`
      );

      actionBtn.setAttribute("href", "#take-action-section");
      const targetId = actionBtn.getAttribute("href");
      document.querySelector(targetId).scrollIntoView({
        behavior: "smooth",
      });
    }
